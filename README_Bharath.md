...
## Formation Control — Spelling **BHARATH** with a 20-Agent Multi-Robot System
...
the letter **B**, then re-forms into **H**, and so on until the final letter
**H**. The full sequence is recorded as an animation and the code + video are
...
multiple sub-contours, so letters with enclosed loops — e.g. the counters in
`B`, `A`, and `R` — are handled correctly). Tracing the outline rather than
filling the interior is what keeps each letter legible with only 20 agents.
...
```
.
├── Q1_Formation_Control_Bharath.py
├── figures/
│   ├── q1_bharath_communication_graph.png     # Erdos–Renyi topology, N=20
│   ├── q1_bharath_formation_snapshots.png     # converged formation per letter
│   ├── q1_bharath_convergence.png             # formation-error vs. time
│   └── q1_bharath_formation_animation.mp4     # full sequence animation (falls back to .gif)
└── README.md
```
...
```bash
python Q1_Formation_Control_Bharath.py
```
...
- **Animation** (`q1_bharath_formation_animation.mp4`) — the full letter-by-letter
